...
def get_max_epoch(log_path):
    """更高效的行数统计版本"""
    try:
        train_logs = np.loadtxt(log_path, skiprows=1)
        return np.max(train_logs[:,0])
    except:
        return 0

def main():
    datasets = ['NC_all', 'HP_15', 'NC_9', 'Ocean', 'Algae'] # 'Ocean', 'Algae', 'NC_9', 'HP_15', 'NC_all'
    
    # 生成所有参数组合（保留dataset和latent_dim的循环）
    all_args = []
    gpu_cycle = itertools.cycle([2,3,1,2,0])
    ...
    # 控制并行数量（根据GPU数量调整）
    print(f'======== {len(all_args)} are retained for AE pretrain ========')
